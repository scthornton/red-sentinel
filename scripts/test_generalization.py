...
def generate_synthetic_test_data(n_samples: int = 1000) -> pd.DataFrame:
    ...
    synthetic_data = pd.DataFrame({
        ...
        'final_label': np.random.choice(['success', 'failure'], n_samples, p=[0.7, 0.3])
    })

    print(f"✅ Generated synthetic data with {len(synthetic_data)} samples")
    print(f"   Models: {synthetic_data['model_name'].nunique()} unique")
    print(
        f"   Techniques: {synthetic_data['technique_category'].nunique()} unique")
    print(
        f"   Label distribution: {synthetic_data['final_label'].value_counts().to_dict()}")
...
def test_generalization():
    ...
    for name, extractor in extractors.items():
        ...
        try:
            ...
            X_real, y_real = extractor.prepare_dataset(
                real_df, is_training=True)
            print(f"   Real data features: {X_real.shape}")

            # Extract features from synthetic data (testing)
            X_synthetic, y_synthetic = extractor.prepare_dataset(
                synthetic_df, is_training=False)
            ...
            print(
                f"   Real data validation - F1: {val_f1:.3f}, Accuracy: {val_acc:.3f}")

            # Test on synthetic data (generalization test)
            y_synthetic_pred = rf.predict(X_synthetic)
            synthetic_f1 = f1_score(
                y_synthetic, y_synthetic_pred, zero_division=0)
            synthetic_acc = accuracy_score(y_synthetic, y_synthetic_pred)

            print(
                f"   Synthetic data test - F1: {synthetic_f1:.3f}, Accuracy: {synthetic_acc:.3f}")
            ...
            print(
                f"   Generalization gap - F1: {f1_gap:+.3f}, Accuracy: {acc_gap:+.3f}")
    ...
    for name, result in results.items():
        ...
        print(
            f"   Real data performance: F1={result['real_f1']:.3f}, Acc={result['real_acc']:.3f}")
        print(
            f"   Synthetic data performance: F1={result['synthetic_f1']:.3f}, Acc={result['synthetic_acc']:.3f}")
        print(
            f"   Generalization gap: F1={result['f1_gap']:+.3f}, Acc={result['acc_gap']:+.3f}")
    ...
    if all('error' not in result for result in results.values()):
        best_extractor = min(
            results.keys(), key=lambda x: results[x]['f1_gap'])
